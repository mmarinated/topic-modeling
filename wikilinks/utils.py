...
def test_model(loader, model, device, threshold=0.5):
    ...
    with torch.no_grad():
        for data1, length1, data2, length2, labels in loader:
            data_batch_1, length_batch_1, data_batch_2, length_batch_2, label_batch = data1.to(device), length1.to(device), data2.to(device), length2.to(device), labels.float()
            outputs_bc = torch.sigmoid(model(data_batch_1, length_batch_1, data_batch_2, length_batch_2))
