...
def get_monolingual_datasets(LANGUAGE, max_num_tokens=None, SAVE=False, LOAD=True):
    """
    @returns
...
#     dict_of_dfs["train_ru_hi"] = wiki_train_ru_hi
    ...
    dict_wiki_tensor_dataset = create_dict_of_tensor_datasets(dict_of_dfs, word_to_index, max_num_tokens=max_num_tokens)
    ...
    #Creating the weight matrix for pretrained word embeddings
    # 0 - <pad>, 1 - <unk> 
    weights_matrix_ve = torch.zeros(len(index_to_word), LANGUAGES_DICT[LANGUAGE]["weights_matrix_ve"].shape[1])
    start_idx = 2
    for language, lang_dict in LANGUAGES_DICT.items():
...
def get_mixed_datasets(LANGUAGES_LIST=("english", "russian", "hindi"), max_num_tokens=None, SAVE=False, LOAD=True):
    """
    @returns
        index_to_word, word_to_index, dict_wiki_tensor_dataset, weights_matrix_ve, classes
    """
